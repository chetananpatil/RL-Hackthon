...
def run(episodes, is_training=True, render=False):
    ...
    if(is_training):
        q = np.zeros((env.observation_space.n, env.action_space.n)) # init a 16 x 16 array
    else:
    ...
    epsilon = 0.5       # 1 = 100% random actions
...
if __name__ == '__main__':
    run(15000, is_training=True,render=False)  # Training
    run(5, is_training=False, render=True)  # Testing with rendering
